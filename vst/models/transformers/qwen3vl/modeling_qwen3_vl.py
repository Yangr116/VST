...
import time
from veomni.data.constants import IGNORE_INDEX, IMAGE_INPUT_INDEX, VIDEO_INPUT_INDEX
...
# torch_npu conv3d has a performance bug with CANN < 8.1
# This is a workaround from Ascend to work around.
class Conv3dWithMatmul(nn.Conv3d):
    ...
    def _conv_forward(self, input, weight):
        ...
        # (N, Ci, D, H, W) -> (N, Ci*D*H*W)
        input_flat = input.reshape(N, -1)

        # (Co, Ci, D, H, W) -> (Co, Ci*D*H*W)
        weight_flat = weight.reshape(Co, -1)

        # (N, Ci*D*H*W) * (Ci*D*H*W, Co) -> (N, Co)
        output = torch.matmul(input_flat, weight_flat.t())

        if self.bias is not None:
            output.add_(self.bias.unsqueeze(0))

        output = output.reshape(N, Co, 1, 1, 1)

        return output

    def _support_conv_to_matmul(self, input):
        return (
            self.kernel_size[0] == input.shape[-3]
            and self.kernel_size[1] == input.shape[-2]
            and self.kernel_size[2] == input.shape[-1]
    ...
    def forward(self, input):
        if self._support_conv_to_matmul(input):
...
class Qwen3VLVisionPatchEmbed(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.proj = Conv3dWithMatmul(self.in_channels, self.embed_dim, kernel_size=kernel_size, stride=kernel_size, bias=True)
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        if position_ids.shape[1] == 3:
            # position_ids' shape = [bs, 3, position]
            position_ids = position_ids.permute(1,0,2) # [3, bs, position]
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if async_enabled:
            ...
            if "position_ids" in kwargs:
                if kwargs["position_ids"].shape[0] == 3:
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        image_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Pre-computed mask indicating the positions of image tokens in the input sequence. This mask is used to
            support sequence parallelism (Ulysses) by avoiding all-gather operations. Required when `pixel_values` is
            not None. The mask should be computed in `process_sample` before calling forward.
        video_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Pre-computed mask indicating the positions of video tokens in the input sequence. This mask is used to
            support sequence parallelism (Ulysses) by avoiding all-gather operations. Required when `pixel_values_videos`
            is not None. The mask should be computed in `process_sample` before calling forward.
        """
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        ...
        if (pixel_values is not None):
            assert image_mask is not None, "image_mask should have already been computed in process_sample"
        
        if (pixel_values_videos is not None):
            assert video_mask is not None, "video_mask should have already been computed in process_sample"
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if "image_mask" in kwargs:
            model_inputs["image_mask"] = kwargs["image_mask"]
        if "video_mask" in kwargs:
            model_inputs["video_mask"] = kwargs["video_mask"]
